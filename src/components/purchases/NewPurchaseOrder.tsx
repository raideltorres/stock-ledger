...
type NewPurchaseOrderValues = z.infer<typeof newPoTransSchema>;
...
export default function NewPurchaseOrder({
  ...
  const form = useForm<NewPurchaseOrderValues>({
  ...
  const handleNewPurchaseOrder = useCallback(
    async (data: NewPurchaseOrderValues) => {
      try {
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleNewPurchaseOrder)}
            className="space-y-6"
          >
            <FormField
