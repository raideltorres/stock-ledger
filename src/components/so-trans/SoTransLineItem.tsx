...
import type { Action, ActionData, SoTransLine } from "@/utils/types";
...
import EditSoTransLine from "./EditSoTransLine";
import DeleteSoTransLine from "./DeleteSoTransLine";

export interface SoTransLineItemProps {
  index: number;
  line: SoTransLine;
  editable?: boolean;
  onAction?: (index: number, data: ActionData<SoTransLine>) => void;
}

export function SoTransLineItem({
  index,
  line,
  editable = false,
  onAction,
}: SoTransLineItemProps) {
  const handleOnAction = useCallback(
    (partial: SoTransLine, action: Action) => {
  ...
  return (
    <Card className="w-full flex flex-col sm:flex-row flex-wrap items-center justify-between gap-2 sm:gap-5 p-2 cursor-pointer">
      ...
      <div className="grid flex-1 text-left text-sm leading-tight">
        ...
        <span className="text-muted-foreground truncate text-xs">
          {`precio: ${(line.unitPrice ?? 0).toLocaleString("en-US", {
            style: "currency",
            currency: "USD",
          })}`}
        </span>
