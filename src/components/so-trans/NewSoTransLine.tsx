import type { NewSoTransLineData } from "@/utils/types";
...
const newSoTransLineSchema = z.object({
...
type NewSoTransLineValues = z.infer<typeof newSoTransLineSchema>;

export interface NewSoTransLineProps {
  onNewSoTransLine?: (data: NewSoTransLineData) => void;
}

export default function NewSoTransLine({
  onNewSoTransLine,
}: NewSoTransLineProps) {
  const toast = useToast();
  const form = useForm<NewSoTransLineValues>({
    resolver: zodResolver(newSoTransLineSchema),
    defaultValues: {
      product: "",
      barcode: "",
      name: "",
  ...
  const handleNewSoTransLine = useCallback(
    async (data: NewSoTransLineValues) => {
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleNewSoTransLine)}
            className="space-y-6"
          >
            <FormField
