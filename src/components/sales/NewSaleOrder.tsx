...
type NewSaleOrderValues = z.infer<typeof newSoTransSchema>;

export interface NewSaleOrderProps {
  locations?: Location[];
  user: User;
  customers?: Provider[];
...
export default function NewSaleOrder({
  ...
  const form = useForm<NewSaleOrderValues>({
  ...
  const handleNewSaleOrder = useCallback(
    async (data: NewSaleOrderValues) => {
      try {
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleNewSaleOrder)}
            className="space-y-6"
          >
            <FormField
